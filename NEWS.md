...
- Add `as_forecast_sample()` and `as_forecast_point()` S3 methods for `baselinenowcast_df` objects, enabling direct conversion of probabilistic or point nowcasts to `scoringutils` forecast objects for scoring against the latest observed counts (#418). `baselinenowcast()` now carries the reporting triangle's `max_delay` as an attribute on the returned `baselinenowcast_df` (retrievable with `get_max_delay()`, now an S3 generic), and the converters use it to score only the right-truncated reference dates that were actually nowcast.
