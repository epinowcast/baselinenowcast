...
-   Add converters between `reporting_triangle` and reviser vintages formats via `as_reviser_vintages()` and `as_reporting_triangle.tbl_pubdate()`, enabling use of reviser's vintage analysis and state space nowcasting methods alongside baselinenowcast's nowcasting functionality (#429).
