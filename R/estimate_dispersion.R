...
estimate_dispersion <- function(
  ...
  for (i in seq_len(n)) { # Seq along retrospective forecast dates
    ...
    for (d in 1:n_horizons) {
      ...
      indices_nowcast <- is.na(replace_lower_right_with_NA(trunc_matr_observed))[max_t - d + 1, ]
      indices_observed <- !is.na(trunc_matr_observed)[max_t - d + 1, ]
      exp_to_add[i, d] <- sum(nowcast_row * indices_nowcast * indices_observed)
      to_add_already_observed[i, d] <- sum(obs_row * indices_nowcast * indices_observed, na.rm = TRUE)
    }
  }
