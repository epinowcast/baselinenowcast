...
test_that("Basic functionality with valid inputs", {
  set.seed(123)
  result <- add_obs_err_to_pt_nowcast_mat(test_matrix, valid_disp)
...
test_that("Random generation is reproducible with seed", {
  set.seed(456)
  result1 <- add_obs_err_to_pt_nowcast_mat(test_matrix, valid_disp)
  set.seed(456)
  result2 <- add_obs_err_to_pt_nowcast_mat(test_matrix, valid_disp)
...
test_that("Output dimensions match input", {
  ...
  disp <- c(2, 2)
  result <- add_obs_err_to_pt_nowcast_mat(square_matrix, disp)
