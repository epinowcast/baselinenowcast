...
test_that("Basic functionality with valid inputs", {
  set.seed(123)
  result <- add_obs_errs_to_pt_nowcast_mat(test_matrix, valid_disp, n_draws)
...
test_that("Input validation works correctly", {
  ...
  expect_error(add_obs_errs_to_pt_nowcast_mat(
  ...
  # Invalid n_draws
  expect_error(add_obs_errs_to_pt_nowcast_mat(test_matrix,
    valid_disp,
    n_draws = -5
  ))
  expect_error(add_obs_errs_to_pt_nowcast_mat(test_matrix,
  ...
  expect_error(add_obs_errs_to_pt_nowcast_mat(test_matrix, valid_disp, 0))

  # dispersion insufficient
  expect_error(add_obs_errs_to_pt_nowcast_mat(test_matrix, c(1, 2), 1))
})

### Test 3: Stochastic Behavior ------------------------------------------------
test_that("Random generation is reproducible with seed", {
  set.seed(456)
  result1 <- add_obs_errs_to_pt_nowcast_mat(test_matrix, valid_disp, 5)
  set.seed(456)
  result2 <- add_obs_errs_to_pt_nowcast_mat(test_matrix, valid_disp, 5)
  expect_identical(result1, result2)
})

### Test 4: Default Parameter Handling -----------------------------------------
test_that("Default n_draws works correctly", {
  default_result <- add_obs_errs_to_pt_nowcast_mat(test_matrix, valid_disp)
...
test_that("Upper triangle remains unchanged", {
  set.seed(789)
  result <- add_obs_errs_to_pt_nowcast_mat(test_matrix, valid_disp, 5)
...
test_that("Lower triangle shows expected variation", {
  set.seed(111)
  results <- add_obs_errs_to_pt_nowcast_mat(test_matrix, c(100, 100, 100), 100)
