test_that("allocate_reference_times works as expected when user specifies training volume precisely and correctly. ", { # nolint
  rep_tri <- make_test_triangle(nrow = 12, ncol = 5, type = "no_nas") |>
    construct_triangle()
  ...
  rep_tri2 <- make_test_triangle(nrow = 14, ncol = 5, type = "no_nas") |>
    construct_triangle()
  ...
  # works
  rep_tri3 <- make_test_triangle(nrow = 10, ncol = 5, type = "no_nas") |>
    construct_triangle()
  tv3 <- expect_warning(
...
test_that("allocate_reference_times properly scales delay and total training amount", { # nolint
  rep_tri <- make_test_triangle(nrow = 20, ncol = 5, type = "no_nas") |>
    construct_triangle()
...
test_that("allocate_reference_times handles rounding with a warning", {
  rep_tri <- make_test_triangle(nrow = 20, ncol = 6, type = "no_nas") |>
    construct_triangle()
  ...
  rep_tri2 <- make_test_triangle(nrow = 100, ncol = 31, type = "no_nas") |>
    construct_triangle()
...
test_that("allocate_reference_times warns when user or defaults don't meet minimum requirements, and reallocates accordingly.", { # nolint
  ...
  rep_tri5 <- make_test_triangle(nrow = 10, ncol = 5, type = "no_nas") |>
    construct_triangle()
  tv5 <- expect_warning(
    allocate_reference_times(
  ...
  rep_tri6 <- make_test_triangle(nrow = 8, ncol = 5, type = "no_nas") |>
    construct_triangle()
  tv6 <- expect_warning(
    allocate_reference_times(
  ...
  rep_tri7 <- make_test_triangle(nrow = 7, ncol = 5, type = "no_nas") |>
    construct_triangle()
  tv7 <- expect_warning(
    allocate_reference_times(
  ...
  rep_tri8 <- make_test_triangle(nrow = 7, ncol = 5, type = "no_nas") |>
    construct_triangle()
  tv8 <- expect_warning(
    allocate_reference_times(
...
test_that("allocate_reference_times errors when data is insufficient. ", { # nolint
  ...
  rep_tri1 <- make_test_triangle(nrow = 10, ncol = 5, type = "no_nas") |>
    construct_triangle()
  expect_error(
    allocate_reference_times(
  ...
  rep_tri2 <- make_test_triangle(nrow = 6, ncol = 5, type = "no_nas") |>
    construct_triangle()
  expect_error(
    allocate_reference_times(
      reporting_triangle = rep_tri2,
...
test_that("allocate_reference_times allocates properly with no user specifications (using defaults)", { # nolint
  rep_tri3 <- make_test_triangle(nrow = 12, ncol = 5, type = "no_nas") |>
    construct_triangle()
  tv <- allocate_reference_times(
    reporting_triangle = rep_tri3
  )
  ...
  rep_tri2 <- make_test_triangle(nrow = 13, ncol = 5, type = "no_nas") |>
    construct_triangle()
  tv2 <- allocate_reference_times(
    reporting_triangle = rep_tri2
  )
  ...
  rep_tri4 <- make_test_triangle(nrow = 8, ncol = 5, type = "no_nas") |>
    construct_triangle()
  tv4 <- expect_warning(
    allocate_reference_times(
  ...
  # Same idea as above
  rep_tri5 <- make_test_triangle(nrow = 9, ncol = 6, type = "no_nas") |>
    construct_triangle()
  tv5 <- expect_warning(
    allocate_reference_times(
      reporting_triangle = rep_tri5
  ...
  rep_tri6 <- make_test_triangle(nrow = 60, ncol = 40, type = "no_nas") |>
    construct_triangle()
  tv6 <- expect_warning(
    allocate_reference_times(
      reporting_triangle = rep_tri6
  ...
  rep_tri7 <- make_test_triangle(nrow = 55, ncol = 39, type = "no_nas") |>
    construct_triangle()
  tv7 <- expect_warning(
    allocate_reference_times(
      reporting_triangle = rep_tri7
  ...
  rep_tri8 <- make_test_triangle(nrow = 120, ncol = 41, type = "no_nas") |>
    construct_triangle()
  ...
  rep_tri9 <- make_test_triangle(nrow = 140, ncol = 41, type = "no_nas") |>
    construct_triangle()
...
test_that("allocate_reference_times warns and reallocates appropriately when sufficient ref times but not enough retro nowcasts", { # nolint
  # Handle larger numbers
  rep_tri <- make_test_triangle(nrow = 14, ncol = 4, type = "no_nas") |>
    construct_triangle()
  tv <- expect_message(
    allocate_reference_times(
      reporting_triangle = rep_tri,
