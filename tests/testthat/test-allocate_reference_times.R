test_that("allocate_reference_times works as expected when user specifies training volume precisely and correctly. ", { # nolint
  tv <- allocate_reference_times(
    reporting_triangle = matrix(data = 1,
                                nrow = 12,
                                ncol = 5) |> construct_triangle(),
    max_delay = 4,
    scale_factor = 3,
    prop_delay = 0.5
  )
 ...
 # works
  ...
  regexp = "Insufficient reference times in reporting triangle for the specified `scale_factor`.") #nolint
...
test_that("allocate_reference_times warns when user or defaults don't meet minimum requirements, and reallocates accordingly.",{ #nolint
  ...
  tv5 <- expect_warning(allocate_reference_times(
    reporting_triangle = matrix(data = 1,
                                nrow = 10,
                                ncol = 5) |> construct_triangle(),
  ),
  ...
  tv6 <- expect_warning(allocate_reference_times(
    reporting_triangle = matrix(data = 1,
                                nrow = 8,
                                ncol = 5) |> construct_triangle(),
  ),
  ...
  tv7 <- expect_warning(allocate_reference_times(
    reporting_triangle = matrix(data = 1,
                                nrow = 7,
                                ncol = 5) |> construct_triangle(),
    prop_delay = 0.9
  ),
  regexp = "`prop_delay` specified is not equivalent to `prop_delay` used.") #nolint
  expect_identical(tv7$n_history_delay, 5)
  expect_identical(tv7$n_retrospective_nowcasts, 2)

  # Prop delay is too low
  tv8 <- expect_warning(allocate_reference_times(
    reporting_triangle = matrix(data = 1,
                                nrow = 7,
                                ncol = 5) |> construct_triangle(),
    prop_delay = 0.3
...
test_that("allocate_reference_times errors when data is insufficient. ", { # nolint
  ...
  expect_error(allocate_reference_times(
    reporting_triangle = matrix(data = 1,
                                nrow = 10,
                                ncol = 5) |> construct_triangle(),
    max_delay = 4,
  ...
  regexp = "Insufficient reference times specified by `scale_factor` for the both delay and uncertainty estimation.") #nolint
...
test_that("allocate_reference_times allocates properly with no user specifications (using defaults)", { # nolint
  ...
  tv6 <- allocate_reference_times(
    reporting_triangle = matrix(data = 1,
                                      nrow = 55,
                                      ncol = 39) |> construct_triangle()
)
