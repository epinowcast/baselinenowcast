data_df <- data.frame(
  time = rep(c(1:10), 3),
  draw = rep.int(1:3, times = 10),
  pred_count = sample(1:20, 30, replace = TRUE)
)
ref_dates <- seq(from = ymd("2025-01-01"), to = ymd("2025-01-10"), by = "day")
strata_list <- list(age_group = "00+", region = "south")
test_that(".combine_data() creates a dataframe from a set of dates and named list of strata", { # nolint
  new_df <- .combine_data(data_df,
    reference_dates = ref_dates,
    strata_list = strata_list
  )

  expected_cols <- c("age_group", "region")

  expect_s3_class(new_df, "data.frame")
  expect_identical(nrow(new_df), 30L)

  expect_true(all(expected_cols %in% colnames(new_df)))

  # time corresponds to reference dates starting at 1
  expected_dates <- min(ref_dates) + days(unique(data_df$time) - 1)
  expect_identical(expected_dates, unique(new_df$reference_date))
  expect_identical(new_df$age_group, rep("00+", 30))
  expect_identical(new_df$region, rep("south", 30))


  # no additional columns if no strata are specified

  new_df_ns <- .combine_data(data_df,
    reference_dates = ref_dates,
    strata_list = NULL
  )
  expected_cols <- c("draw", "pred_count", "reference_date")
  expect_identical(expected_cols, colnames(new_df_ns))
})

test_that(".combine_data correctly merges reference dates", {
  # Setup
  nowcast_df <- data.frame(
    time = c(1, 2, 3),
    pred_count = c(100, 200, 300),
    draw = c(1, 1, 1)
  )
  strata_list <- NULL
  reference_dates <- as.Date(c("2024-01-01", "2024-01-02", "2024-01-03"))

  result <- .combine_data(nowcast_df, strata_list, reference_dates)

  expect_identical(result$reference_date, reference_dates)
  expect_identical(result$pred_count, c(100, 200, 300))
})

test_that(".combine_data removes time column from output", {
  # Setup
  nowcast_df <- data.frame(
    time = c(1, 2, 3),
    pred_count = c(10, 20, 30),
    draw = c(1, 1, 1)
  )
  strata_list <- list(age_group = "0-17")
  reference_dates <- as.Date(c("2024-01-01", "2024-01-02", "2024-01-03"))
  result <- .combine_data(nowcast_df, strata_list, reference_dates)

  expect_false("time" %in% names(result))
})

test_that(".combine_data handles non-sequential time values", {
  # Setup
  nowcast_df <- data.frame(
    time = c(1, 3, 5),
    pred_count = c(10, 30, 50),
    draw = c(1, 1, 1)
  )
  strata_list <- list(region = "East")
  reference_dates <- as.Date(c(
    "2024-01-01", "2024-01-02", "2024-01-03",
    "2024-01-04", "2024-01-05"
  ))

  result <- .combine_data(nowcast_df, strata_list, reference_dates)

  expect_identical(nrow(result), 3L)
  expect_identical(
    result$reference_date,
    as.Date(c("2024-01-01", "2024-01-03", "2024-01-05"))
  )
})
