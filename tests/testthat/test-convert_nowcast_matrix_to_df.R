...
result <- convert_nowcast_matrix_to_df(test_matrix)
...
test_that("convert_nowcast_matrix_to_df works correctly", {
  ...
  # With draw specified
  result_draw <- convert_nowcast_matrix_to_df(test_matrix, draw = 5)
  ...
  result_single <- convert_nowcast_matrix_to_df(single_row)
  ...
  na_matrix[2, 2] <- NA
  result_na <- convert_nowcast_matrix_to_df(na_matrix)
  expect_true(anyNA(result_na$count))

  ### Test 6: Input Validation -------------------------------------------------
  # Non-matrix input, should still work
  expect_silent(convert_nowcast_matrix_to_df(as.data.frame(test_matrix)))

  # Invalid draw type
  expect_error(convert_nowcast_matrix_to_df(test_matrix, draw = "invalid"))
})

test_that("Column ordering and naming works correctly", {
  # Matrix with named columns
  named_matrix <- matrix(1:4, nrow = 2)
  result_named <- convert_nowcast_matrix_to_df(named_matrix)
...
test_that("Large matrix handling", {
  ...
  result_large <- convert_nowcast_matrix_to_df(large_matrix)
