...
In this case study, we will use synthetic data from the United States' [National Syndromic Surveillance](https://www.cdc.gov/nssp/index.html) (NSSP) dataset.
...
Once we have shown how to generate data of this form from the line-list data, we will swap out the dataframe for a larger synthetic dataset formatted the same way, and demonstrate how to use `baselinenowcast` to generate a nowcast of the admissions.
...
We will start by loading in the synthetic line-list dataset.
...
```{r}
df_all <- merge(syn_nssp_time_stamps_long,
  syn_nssp_diagnoses_long,
  by = "event_id"
) |>
