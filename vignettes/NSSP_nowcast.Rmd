...
These could easily be swapped out with another set of diangoses codes e.g. for influenza-like illness, COVID-19, etc.
...
expand_events <- function(line_list, event_col_name) {
  wide_line_list <- line_list |>
    separate_wider_delim({{ event_col_name }},
...
wide_to_long <- function(wide_line_list,
                         ...
                         values_to_name,
                         id_col_name) {
  lastcol_ <- tail(colnames(select(
    wide_line_list,
    matches({{ event_col_name }})
  )), 1)
  long_data <- wide_line_list |>
    ...
    mutate(
      event_id = paste(
        .data[[id_col_name]],
        as.numeric(str_extract(as.character(column_name), "[0-9.]+"))
      )
    )
  return(long_data)
}
...
syn_nssp_diagnoses_long <- wide_to_long(
  ...
  event_col_name = "DischargeDiagnosisUpdates",
  values_to_name = "diagnoses_codes",
  id_col_name = "C_Processed_BioSense_ID"
)
```
...
First, we generate the point estimate of the nowcast as a "filled in" reporting triangle by estimating a reporting delay and applying it to the partially observed case counts.
