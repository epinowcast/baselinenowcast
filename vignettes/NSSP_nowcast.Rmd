...
For nowcasting, we want to compute the number of incident cases indexed by referene and report date, so we can aggregate by reference and report date and compute the delay.
```{r}
...
We'll start by generating a reporting triangle from the `syn_nssp_rep_tri_df`, after we've removed all of the reports from after the nowcast date.
...
We will use the helper function `allocate_reference_times()` to determine how many reference times to use for delay estimation and uncertainty estimation.
We will use the defaults for this function, which uses 3 times the maximum delay as the total amount of training volume, split evenly between reference times used for delay estimation and reference times used for producing retrospective nowcasts for uncertainty estimation.
In this case, this will result in 37 reference times used for delay estimation, and 38 used for uncertainty estimation.
```{r}
...
Here we demonstrate the workflow using the intermediate-level functions. For more details, see the [Getting Started](baselinenowcast.html) vignette.
