...
Next we obtain the reporting triangle.
First we preprocess the data to ensure it is in the correct format and has the required columns.
...
We only have one group here so we only need `reference_date`, `report_date`, and `new_confirm`.

```{r}
reporting_triangle_df <- pobs$new_confirm[[1]] |>
  select(
    reference_date,
    report_date,
    count = new_confirm
...
We now create a reporting_triangle object directly from the data frame.
This handles the conversion from long format to the matrix representation needed for nowcasting.

```{r}
reporting_triangle <- as_reporting_triangle(
  data = reporting_triangle_df,
...
We will specify the number of observations we want to use for estimation.
The maximum delay is determined by the reporting triangle structure.
...
obs_with_nowcast_draws_df <- nowcast_draws_df |>
  left_join(latest_data_prepped, by = "reference_date") |>
