...
There are two methods we could use to estimate the errors. The first method
simulates exactly the iterative process, by generating retrospective reporting
triangles using what would have been available as of the time, estimating a
delay distribution, and then re-computing the nowcast. To do this, we need to
specify the number of observations we want to use for the estimation of the
dispersion, which we will call `n_history_dispersion`. We will need to have
`n_history_dispersion + n_history` observations to generate these estimates, as
each `n_history_dispersion` will require at least `n_history` rows to
re-estimate the delay distribution. Note that `n_history` must always be
greater than the `max_delay`.

```{r estimate-uncertainty-method1}
disp_params1 <- estimate_uncertainty(
  triangle_for_uncertainty = triangle,
  n_history = 41,
  n_history_dispersion = 18
)
```
