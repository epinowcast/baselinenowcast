...
To do this, we will first generate retrospective reporting triangles using the
latest reporting triangle, and then we will generate nowcasts from the list
of reporting triangles. To generate the nowcasts, we will have to specify the
number of reporting triangles we want to use for the estimation of the
uncertainty, which we will call `n_history_uncertainty`, as well as the number
of observations we want to use to estimate the delay when we generate the
nowcast, `n_history_delay`.

Note, we will need to have more than `n_history_uncertainty + n_history_delay`
observations in our original reporting triangle to estimate the uncertainty,
as each `n_history_uncertainty` will require at least `n_history`
rows to re-estimate the delay distribution. Note also that `n_history_delay`
must always be greater than the `max_delay + 1`.

We'll start by generating the set of retrospective reporting triangles.
Since we have 60 observations in our reporting triangle, we will generate 18
reporting triangles, as we will need at least 42 observations (rows) in each
reporting triangle to estimate a delay distribution with a maximum of 40.


```{r generate-retrospective-triangles}
retro_rts <- generate_retro_triangles(
  triangle = triangle,
  n_triangles = 18
)
```
The `generate_retro_triangles()` function returns a list of
`n_triangles` reporting triangles, in order from the most to least
...
that each triangle will be a different number of rows.

Next we will pass this list of reporting triangles to the
`generate_retro_nowcasts()` alongside the specification of how many
observations to use to estimate the delay, which will default to the minimum
number of rows in all of the matrices in the list of retrospective reporting
triangles, in this case, 42.

We could make this number larger, but that would require passing in only
the reporting triangles with that number or more rows.
```{r}
...
```{r}
# disp_params <- estimate_disp_params(retro_nowcasts) #nolint
```
