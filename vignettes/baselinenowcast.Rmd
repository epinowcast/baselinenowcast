...
triangle <- triangle_full |>
  dplyr::select(-`.group`, -reference_date) |> # remove unnecessary columns
  as.matrix() |>
  unname()
head(triangle)
...
We would like to generate a probabilistic nowcasts, which will require
...
*Note, we could probably create objects of different classes for the outputs
of each of these functions, but I am not sure this is necessary*

```{r}
# disp_params <- estimate_uncertainty( #nolint
# triangle_to_nowcast = triangle #nolint
# delay_pmf = delay_df$pmf #nolint
# ) #nolint
```
