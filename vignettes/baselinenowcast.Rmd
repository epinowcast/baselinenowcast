...
`ggplot2`, `lubridate`, `tidyr`, and `dplyr`.
...
data_long <- germany_covid19_hosp
...
maximum delay for the total training volume, split into 50% used for delay
delay estimation and 50% used for uncertainty estimation.
...
n_history_delay <- 1.5 * n_training_volume

# Specify the number of reference times to use for uncertainty estimation.
n_history_uncertainty <- 1.5 * n_training_volume
```
...
observed_long <- data_long[location == "DE"][age_group == "00+"] |> # nolint
  enw_filter_report_dates(latest_date = nowcast_date) |>
  enw_filter_reference_dates(include_days = n_training_volume - 1) # nolint
head(observed_long)
...
in one single step by calling `generate_pt_nowcast_mat()`. In susbequent steps
...
ggplot() +
  geom_line(
    # Plot the data summed across reporting days as of August 1,2021
    data = summary_data,
...
triangles using what would have been available as of each reference time,
and then for each reporting triangle, estimating a delay distribution and
generating a point nowcast matrix.
...
We will repeat this iterative process for `n_history_uncertainty` reference
times in the reporting triangle, which are treated as retrospective nowcast
times. We start from the latest and work backwards.
...
We will generate a long tidy dataframe containing draws of only the predicted
...
We'll first have to extract from the point nowcast matrix
only the values that were predicted.
```{r extract predictions}
pt_nowcast_pred_matrix <- extract_predictions(
  pt_nowcast_mat = point_nowcast_matrix,
  rep_mat = reporting_triangle
)
```
...
nowcast vectors (grey) compared to the final observed data (black) and the
