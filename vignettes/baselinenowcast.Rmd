...
triangles using what would have been available as of the time, estimating a
delay distribution, and then re-computing the nowcast.

To do this, we will first generate retrospective nowcasts from the reporting
triangle we have. We will have to specify the number of observations we want to
use for the estimation of the uncertainty, which we will call
`n_history_uncertainty`.
Note, we will need to have more than `n_history_uncertainty + n_history`
observations in our original reporting triangle to estimate the uncertainty in
this way, as each `n_history_uncertainty` will require at least `n_history`
rows to re-estimate the delay distribution. Note also that `n_history` must
always be greater than the `max_delay + 1`.

We'll start by generating the set of retrospective reporting triangles.
Since we have 60 observations in our reporting triangle, we will use 19 of them
to estimate the uncertainty and the remaining 41 to estimate the delay
distribution (since we need to have at least 41 observations to estimate a
delay distribution with a maximum of 40).

```{r generate-retrospective-data}
retro_rts <- generate_retrospective_data(
  triangle_for_retro = triangle,
  n_history_uncertainty = 19
)
```
The second method differs slightly, in that instead of re-estimating the
delay distribution each time, we are going to use the same delay distribution
that will be used to generate the nowcast to compute the retrospective nowcasts.
In this case, since we do not need the additional `n_history` observations to
re-estimate the delay distribution, we only need `n_history_dispersion`
observations
```{r estimate-uncertainty-method2}
disp_params2 <- estimate_uncertainty(
