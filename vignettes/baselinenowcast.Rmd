...
point_nowcast_matrix <- apply_delay(
  triangle_to_nowcast = cleaned_reporting_triangle,
  delay_pmf = delay_pmf
)
```
...
truncated_rts <- truncate_triangles(
  triangle = cleaned_reporting_triangle
)
...
```{r generate-nowcasts}
retro_pt_nowcast_matrix_list <- generate_pt_nowcast_mat_list(
...
disp_params <- estimate_dispersion(
  list_of_nowcasts = retro_pt_nowcast_matrix_list,
  list_of_trunc_rts = truncated_rts
)
```
...
nowcast_draw <- add_obs_err_to_pt_nowcast_mat(
...
exp_obs_nowcasts <- add_obs_errs_to_pt_nowcast_mat(
  point_nowcast_matrix = point_nowcast_matrix,
  disp = disp_params,
  n_draws = 1000
)
