...
of obserations we want to use to estimate the delay when we generate the
...
Since we have 60 observations in our reporting triangle, we will generate 19
reporting triangles, as we will need at least 41 observations in each to
estimate a delay distribution with a maximum of 40.
...
`generate_retro_nowcasts()` alongside the specification of how many triangles
to use for the uncertainty estimate and how many observations to use to
estimate the delay
```{r}
# nolint start
