...
if (!requireNamespace("epinowcast", quietly = TRUE)) {
  cli::cli_warn(
    c(
      "These vignettes require epinowcast but it is not installed.",
      paste(
        "Currently epinowcast is not on CRAN but can be installed",
        "from r-universe with:"
      ),
      paste(
        "install.packages('epinowcast',",
        "repos = c('epinowcast.r-universe.dev',",
...
The above example uses the default uncertainty configuration, which
estimates uncertainty using a negative binomial distribution fitted
independently by horizon.
You can customise this behaviour using the `uncertainty_opts()` function
to control the distribution, fitting strategy, and data aggregation.

### Using a Poisson distribution

To use a Poisson distribution instead of negative binomial:

```{r}
# Estimate dispersion with Poisson model
disp_params_pois <- estimate_uncertainty(
  point_nowcast_matrices = retro_pt_nowcast_mat_list,
  truncated_reporting_triangles = trunc_rep_tri_list,
  retro_reporting_triangles = retro_rep_tri_list,
  n = n_retrospective_nowcasts,
...
# Generate nowcasts with Poisson uncertainty
nowcast_draws_pois <- sample_nowcasts(
  point_nowcast_matrix, reporting_triangle,
  uncertainty_params = disp_params_pois,
  draws = 100,
  uncertainty = uncertainty_opts(model = uncertainty_poisson())
)
```
...
You can also specify custom aggregation functions to operate on
reference times or delays before fitting the uncertainty model.
For example, to use a 3-day rolling sum:

```{r, eval = requireNamespace("zoo", quietly = TRUE)}
# Estimate with rolling sum aggregation
...
The uncertainty configuration system ensures that the same
aggregation functions are consistently applied during both
parameter estimation and nowcast generation.
See `?uncertainty_opts` for more details on available options.
