...
scale of days to create the reporting triangle.
In general the delay and the reference times can have any temporal granularity.
...
maximum delay for the total training volume, with the most recent 50% of the
reference times used for delay estimation.
...
max_delay <- 30
```
Specify the total amount of training volume, the number of reference times to
use to estimate the delay distribution and the number of retrospective nowcast
datasets to use for uncertainty estimation.
Note this assumes you want the most recent observations.
...
We can plot the reporting triangle.
...
We'll make a quick plot to compare our point estiamte of the nowcasted
...
recent, starting from the most recent.
It is not filtered to exclude anymrows that may not ultimately be used to
generate the nowcast, which meansmthat each triangle will have a different
number of rows.
