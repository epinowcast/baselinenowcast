...
In this example, we will demonstrate how to specify the reference time allocation and the number of draws we want returned, and will otherwise use the default specifications.
This will by default generate 1000 draws of a probabilistic nowcast.
...
In this vignette we used `baselinenowcast` functions to convert a dataframe with incident cases indexed by reference date and report date to a `reporting_triangle` object using the `as_reporting_triangle` function, and then used the `baselinenowcast()` function to generate a probabilistic nowcast dataframe.
The `baselinenowcast()` function internally ran each of the steps of our nowcasting workflow including estimating a delay distribution, applying it to generate a point nowcast, and estimating and applying uncertainty to generate probabilistic nowcasts.
...
For an example of walking through this same nowcasting problem with the modular workflow, see the [modular workflow](modular_workflow.html) vignette.
<!-- Note, the below relies heavily on the summary in nssp_nowcast, so we could make this a code chunk like installation to avoid duplication -->
...
See the [model definition](model_definition.html) for more details on the mathematical methods.
