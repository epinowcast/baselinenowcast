...
To do this, we will first generate retrospective reporting triangles from the
latest reporting triangle.
We will have to specify the number of observations we want to use for the
estimation of the uncertainty, which we will call `n_history_uncertainty`.
...
using this method, as each `n_history_uncertainty` will require at least
`n_history` rows to re-estimate the delay distribution. Note also that `n_history_delay`
...
delay distribution with a maximum delay of 40).
