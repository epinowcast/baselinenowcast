...
of reporting triangles. This will default to assuming you want to generate
...
Now that we have estimated the dispersion, we can treat the point nowcast we
previously generated, `point_reporting_square` and add observation error to it
for many draws to generate probabilistic nowcasts. We'll start by demonstrating
how to just add observation error to a single poist nowcast.

```{r add-observation-error}
nowcast_draw <- add_obs_error_to_nowcast(
  nowcast = point_reporting_square,
  disp = disp_params
)
```
