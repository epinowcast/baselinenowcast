...
final_data_prepped <- target_data |>
  ...
  mutate(
    ...
    nowcast = rowSums(point_nowcast_matrix)
  )
...
combined_data <- obs_with_nowcast_draws_df |>
  select(reference_date, obs_confirm, final_confirm, nowcast) |>
  distinct() |>
  pivot_longer(
    cols = c(obs_confirm, final_confirm, nowcast),
    names_to = "type",
    values_to = "count"
  ) |>
  mutate(type = case_when(
...
```
Extension: compute a probabilstic nowcast on the 7 day rolling sum
```{r}
disp_params_7d <- estimate_dispersion(
  pt_nowcast_mat_list = retro_pt_nowcast_mat_list,
  trunc_rep_tri_list = trunc_rep_tri_list,
  reporting_triangle_list = retro_rep_tri_list,
  n = n_history_delay,
...
nowcast_draws_df_7d <- get_nowcast_draws(
  point_nowcast_matrix, reporting_triangle,
  dispersion = disp_params,
  draws = 100,
  fun_to_aggregate = sum,
...
head(nowcast_draws_df_7d)

obs_with_nowcast_draws_df_7d <- nowcast_draws_df_7d |>
  left_join(
    latest_data_prepped |>
      mutate(obs_confirm = rollapply(obs_confirm,
        7,
        sum,
        align = "right",
        fill = NA
  ...
  left_join(
    final_data_prepped |>
      mutate(final_confirm = rollapply(final_confirm,
        7,
        sum,
        align = "right",
        fill = NA
...
combined_data_7d <- obs_with_nowcast_draws_df_7d |>
...
ggplot() +
  # Add nowcast draws as thin gray lines
  geom_line(
    data = obs_with_nowcast_draws_df_7d,
    aes(
      x = reference_date, y = pred_count, group = draw,
      color = "Nowcast draw", linewidth = "Nowcast draw"
    )
  ...
  scale_y_continuous(trans = "log10") +
  xlab("Reference date") +
  ylab("7-day rolling sum hospital admissions") +
  theme(legend.position = "bottom") +
  ggtitle("Comparison of 7-day sum of admissions as of the nowcast date, \n later observed counts, and probabilistic nowcasted counts") # nolint

# Lets compute quantiles so that we can recreate the figure in Wolffram et al

summary_estimates <- obs_with_nowcast_draws_df_7d |>
  group_by(reference_date) |>
  summarise(
    q_50 = quantile(pred_count, 0.5, na.rm = TRUE),
    q_lb_50th = quantile(pred_count, 0.25, na.rm = TRUE),
    q_ub_50th = quantile(pred_count, 0.75, na.rm = TRUE),
    q_lb_95th = quantile(pred_count, 0.025, na.rm = TRUE),
    q_ub_95th = quantile(pred_count, 0.975, na.rm = TRUE)
  )

ggplot() +
  # Add nowcast draws as thin gray lines
  geom_line(
    data = summary_estimates,
    aes(
      x = reference_date, y = q_50,
      color = "Nowcast draw"
    )
  ) +
  geom_ribbon(
    data = summary_estimates,
    aes(
      x = reference_date, ymin = q_lb_50th, ymax = q_ub_50th,
      color = "Nowcast draw", alpha = 0.2
    )
  ) +
  geom_ribbon(
    data = summary_estimates,
    aes(
      x = reference_date, ymin = q_lb_95th, ymax = q_ub_95th,
      color = "Nowcast draw", alpha = 0.2
    )
  ) +
  # Add observed data and final data once
