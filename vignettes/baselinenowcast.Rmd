...
We now pivot to wide format to create a matrix, then convert to a reporting_triangle object.

```{r}
reporting_triangle_mat <- reporting_triangle_df |>
  pivot_wider(names_from = delay, values_from = new_confirm) |>
  select(-reference_date) |>
  as.matrix()

reference_dates <- unique(reporting_triangle_df$reference_date)

reporting_triangle <- as_reporting_triangle(
  data = reporting_triangle_mat,
...
```{r}
triangle_df <- as.data.frame(unclass(reporting_triangle)) |>
