...
The `get_delay_estimate()` function expects the following inputs:
  ...
  of rows in `triangle`.
`enw_preoprocess()` returns a `triangle` with the columns `.group` and
...
we will start by demonstrating the functionality on the raw matrix of the
reporting triangle.
...
```{r}
# reporting_square <- apply_delay( #nolint
