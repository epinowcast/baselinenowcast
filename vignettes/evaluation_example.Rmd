...
We will set up a for loop to retrospectively produce nowcasts at each of the specified nowcast dates.
...
3. Generating the data to evaluate the nowcast using the latest observations. We'll only include reports that occurred within the maximum delay window to ensure a fair comparison across all nowcast dates, as including all reports would mean that earlier nowcast dates are compared against longer delays than more recent nowcast dates. See our [publication](https://wellcomeopenresearch.org/articles/10-614) and [Wolffram et al.](https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1011394) for further details.
...
run_single_nowcast <- function(nowcast_date, prop_delay, scale_factor,
  ...
  initial_count_data <- training_data |>
    ...
    dplyr::summarise(initial_count = sum(count)) |>
    dplyr::filter(reference_date >=
      max(reference_date) - lubridate::days(max_delay))

  eval_data <- data |>
    dplyr::mutate(delay = as.integer(report_date - reference_date)) |>
    dplyr::filter(delay <= max_delay, reference_date < nowcast_date) |>
    dplyr::group_by(reference_date) |>
    dplyr::summarise(final_count = sum(count)) |>
    dplyr::ungroup()

  rep_tri <- baselinenowcast::as_reporting_triangle(training_data) |>
    baselinenowcast::truncate_to_delay(max_delay = max_delay)

  nowcast_df <- baselinenowcast::baselinenowcast(rep_tri,
    scale_factor = scale_factor,
    prop_delay = prop_delay,
    draws = 1000
  )

  su_draws <- scoringutils::as_forecast_sample(
    nowcast_df,
  ...
  su_quantiles <- su_draws |>
    ...
    dplyr::left_join(initial_count_data, by = "reference_date")
...
mult_nowcasts <- map_dfr(
  ...
  in_parallel(
    ...
    run_single_nowcast = run_single_nowcast,
    prop_delay = prop_delay,
    scale_factor = scale_factor,
    syn_nssp_df = syn_nssp_df,
    max_delay = max_delay,
    .library = c("baselinenowcast", "dplyr", "scoringutils", "lubridate")
  ),
  .id = "nowcast_date",
  .progress = TRUE
) |>
...
```{r, message = FALSE, eval = !file.exists(file.path("..", "man", "figures", "nowcast_flipbook.gif"))}
...
In this vignette and elsewhere, we evaluate the predicted final case counts against the observed final case counts, similar to [Wolffram et al](https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1011394) and [Johnson et al.](https://wellcomeopenresearch.org/articles/10-614).
...
This can be easily performed on the nowcast quantiles using the `scoringutils` function `transform_forecast(fun == log_shift, offset = 1, append = FALSE)`, see [`scoringutils` documentation](https://epiforecasts.io/scoringutils/reference/transform_forecasts.html) for implementation details.
For more information on the implications of scoring epidemiological forecasts on transformed scales, see [Bosse et al.](https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1011393).
...
We can use the `pmap_dfr` function from the `purrr` package to iterate through each combination of nowcast dates, proportion of data used for delay estimation, and the total number of reference dates used for training, using the same function we previously defined [above](evaluation_example.html/looping-over-multiple-nowcast-dates) `run_single_nowcast()`.
...
We next score the quantiles for all the nowcast dates and training volume specifications.
```{r}
