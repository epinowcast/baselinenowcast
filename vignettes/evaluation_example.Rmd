...
mult_nowcasts <- map_dfr(
  ...
  \(nowcast_date) {
    # Create training data as of the nowcast date
...
mult_nowcasts <- mutate(mult_nowcasts,
...
p_anim <- p_all_dates +
  ...
  ease_aes("linear")
knitr::include_graphics(file.path("..", "man", "figures", "nowcast_flipbook.gif"))
```
...
In our loop, we will follow the same steps as described [above](evaluation_example.html/looping-over-multiple-nowcast-dates)
...
mult_nowcasts_ms <- pmap_dfr(param_grid, \(nowcast_date,
  prop_delay,
  scale_factor) {
  # Create training data as of the nowcast date
  training_data <- get_training_data(
