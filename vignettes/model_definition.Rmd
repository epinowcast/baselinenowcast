...
We take the list of point nowcast matrices, and the list of truncated incomplete reporting matrices (these do not necessarily contains NAs on the bottom right, the bottom right could be entirely or partially observed).
For each retrospective nowcast time $s^*$, we identify the overlapping set of matrix elements that were estimated and the matrix elements that had been observed as of the most recent time point.

We choose to quantify the uncertainty at the level of the target quantity which is in this case assumed to be the final count at each reference time, summed across reporting delays.
We estimate the dispersion by comparing the predicted counts summed across delays to the observed counts summed across delays, conditioned on the counts having been observed as of time $t*$.

At each retrospective nowcast time $s^*$, we compute the predicted and corresponding observed nowcast at each forecast horizon $j = 1, ..., D$ by summing across the reporting delays for all delays $d$ that have been observed as of time $t^*$, as indicated below by the indicator function.
We define the predicted nowcast for forecast horizon $j$ at retrospective nowcast time $s^*$
$$
\hat{X}_{t-j}(s^*) = \sum_{d=0}^{D} \hat{X}_{t-j,d}(s^*) \times I(t-j+d \leq t^*)
$$
and likewise the observed nowcast for forecat horizon $j$ at retrospective nowcast time $s^*$
$$
...
The function `estimate_dispersion()` ingests a list of truncated reporting matrices representing the observations, and the list of point nowcast matrices, and returns a vector of negative binomial dispersion parameters indexed starting at forecast horizon $j = 1$.
...
We can then join this to the original data in long form by reference date and reporting date in order to generate the probabilistic nowcasts as the sum of the predicted total count from each draw and the observed count for each reference time as of the final reference time.
