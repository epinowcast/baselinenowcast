...
This can be achieved in a stepwise manner using the functions `truncate_triangles()` and `apply_reporting_structures()` which return a list of `n` retrospective reporting triangles in order from most recent to oldest.

### Generation of retrospective point nowcast matrices

For each of the $M$ vintage reporting triangles, i.e., $s^* = t^*-1,, ..., t^*-M$, we apply the method described above to estimate a delay distribution and generate a point nowcast matrix using the function `estimate_and_apply_delays()`.
