---
title: "Modular workflow for flexible model development"
description: "An example walking through the low-level function interface for building a nowcast model"
output:
...
We will use the same dataset and problem from the [Gettign Started](baselinenowcast.html) vignette, and our assumption is that users have read that and are familiar with concepts introduced there such as the reporting triangle.
...
In the modular workflow, we can specify precisely the number of reference times used for delay estimation and uncertainty estimation.
...
We will use the most recent 50% of the reference times for delay estimation.
...
We will use the `reporting_triangle` class object, `rep_tri` for the remaining workflow steps.
...
Note that the estimation of uncertainty from a reporting triangle can be done in a single step using the `estimate_uncertainty_retro()` function, which chains togeter the truncation of triangles, construction of retrospectie reporting triangles, the generation of point nowcasts via filling the triangles, and the estimation of the uncertainty from the estimates compared to the observations.
...
We'll do this by adding a `time` column to `latest_data`.
...
4. `construct_triangles()` - Constructing retrospective reporting triangles based on what would have been available as of the nowcast date.
...
7. `sample_nowcasts()` - Sampling from the observation model to generate probabilistc nowcasts

This modular workflow allows for maximum flexibility and the ability to interrogate intermediate steps.
The idea is that from these component functions, a wide range of `baselinenowcast` models can be developed via a pipeline-based approach where the outputs of one function are pased to the inputs of another function, or the inputs of one function can for example be swapped with an external estimate.

This vignette walks through and produces the exact same results as in the [Getting Started](baselinenowcast.html) vignette, which uses the package default settings to specidy the model.
...
<!-- Noting redundancy here and above with nssp nowcast and getting started -->
