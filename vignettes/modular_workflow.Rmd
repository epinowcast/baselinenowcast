...
Alternatively, if you just wanted to specify the `n_history_delay` and `n_retrospective nowcasts` directly, you could do this.
...
Here we will use 7 weeks, or 49 days, of data for estimating delay and uncertainty.

```{r}
n_history_delay <- 49
n_retrospective_nowcasts <- 49
```
...
For each truncated triangle, we will use the `construct_triangles()` function to generate what would have been available as of the latest reference time within each truncated triangle.
...
```
T
his function uses a negative binomial observation model by default, and independently estimates the dispersion in the negative binomial at each forecast horizon.
