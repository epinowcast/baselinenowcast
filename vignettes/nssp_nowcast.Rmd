...
```{r}
ref_time_scale_factor <- 3
prop_used_for_delay <- 0.5
```

This means that 25*3, 75 reference times will be used for fitting the model, with about half of them (37) being used for delay estimation and the other half (38) used for uncertainty estimation.
...
This function chains together the `baselinenowcast` workflow, see `?baselinenowcast.reporting_triangle` for more details on each step in the workflow, and check out the [mathematical_model](model_definition.html) for details on the mathematical model used for each component.
...
nowcast_draws_df <- baselinenowcast(rep_tri,
  scale_factor = ref_time_scale_factor,
  prop_delay = prop_used_for_delay,
  draws = 1000
)
